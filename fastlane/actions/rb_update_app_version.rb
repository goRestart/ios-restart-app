...
def self.run(params)
	...
	app_plist_path = File.join(path_to_repo, ENV["APP_PLIST_PATH"])
	notification_plist_path = File.join(path_to_repo, ENV["NOTIFICATION_PLIST_PATH"])

	current_app_build_num = getInfoPlistValue("CFBundleVersion", app_plist_path)
	current_app_version_num = getInfoPlistValue("CFBundleShortVersionString", notification_plist_path)

	if autoincrement
		if build_number.nil?
			build_number = (current_app_build_num.to_i + 1).to_s
		else
			UI.warning "Won't autoincrement because build_number #{build_number} was passed"
    	end
	end
	...
	if current_app_build_num.strip == build_number
		...
		setInfoPlistVersionValue("CFBundleVersion", build_number, app_plist_path)
		setInfoPlistVersionValue("CFBundleVersion", build_number, notification_plist_path)
		sth_changed = true
	end
	...
	if current_app_version_num.strip == version_number
		...
		setInfoPlistVersionValue("CFBundleShortVersionString", version_number, app_plist_path)
		setInfoPlistVersionValue("CFBundleShortVersionString", version_number, notification_plist_path)
		sth_changed = true
	end
...
def self.description
	"Modifies the build_number (either by autoincrementing or by the parameter) and the version_number if provided."
end
