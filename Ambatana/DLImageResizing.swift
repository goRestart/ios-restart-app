...
extension UIImage {
    ...
    func resizedImageToSize(size: CGSize, transform: CGAffineTransform, needsToBeTransposed: Bool, interpolationQuality: CGInterpolationQuality) -> UIImage? {
        ...
        let context = CGBitmapContextCreate(nil, UInt(newFrame.size.width), UInt(newFrame.size.height), CGImageGetBitsPerComponent(imageRef),
    ...
    func roundedCornersImageOfSize(size: CGFloat, borderSize: UInt) -> UIImage? {
        ...
        let context = CGBitmapContextCreate(nil, UInt(image.size.width), UInt(image.size.height), CGImageGetBitsPerComponent(cgImage), 0, CGImageGetColorSpace(cgImage), CGImageGetBitmapInfo(cgImage))
    ...
    func imageWithAlpha() -> UIImage? {
        ...
        let alphaInfoAsBitmapInfo = CGBitmapInfo(CGImageAlphaInfo.PremultipliedFirst.rawValue)
        let context = CGBitmapContextCreate(nil, width, height, 8, 0, CGImageGetColorSpace(cgImage), CGBitmapInfo.ByteOrderDefault | alphaInfoAsBitmapInfo)
        ...
        let cgAlphaImage = CGBitmapContextCreateImage(context)
    ...
    func transparentBorderedImage(borderSize: UInt) -> UIImage? {
        ...
        let cgImage = image!.CGImage
        let context = CGBitmapContextCreate(nil, UInt(newFrame.size.width), UInt(newFrame.size.height), CGImageGetBitsPerComponent(cgImage), 0, CGImageGetColorSpace(cgImage), CGImageGetBitmapInfo(cgImage))
        ...
        let transparentBorderImage = CGImageCreateWithMask(borderImage, maskImage)
    ...
    func newBorderMask(borderSize: UInt, size: CGSize) -> CGImageRef {
        ...
        let alphaInfoAsBitmapInfo = CGBitmapInfo(CGImageAlphaInfo.None.rawValue)
        let context = CGBitmapContextCreate(nil, UInt(size.width), UInt(size.height), 8, 0, colorSpace, CGBitmapInfo.ByteOrderDefault | alphaInfoAsBitmapInfo)
