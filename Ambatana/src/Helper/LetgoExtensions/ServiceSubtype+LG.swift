...
extension Array where Element == ServiceSubtype {
    ...
    func makeCreationParams(imagesIds: [String],
        ...
        return self.enumerated().map { (index, subtype) in
            let serviceAttribute = postListingState.verticalAttributes?.serviceAttributes?.updating(subtypeId: subtype.id) ??
                ServiceAttributes(subtypeId: subtype.id)
...
extension Collection where Element == ServiceSubtype {
    ...
    func makeCellRepresentables() -> [DropdownCellRepresentable] {
        return self.map {
            let cellContent = DropdownCellContent(type: .item(featured: $0.isHighlighted),
                                                  title: $0.name,
                                                  id: $0.id)
