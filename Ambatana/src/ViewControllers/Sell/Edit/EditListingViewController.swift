...
class EditListingViewController: BaseViewController, UITextFieldDelegate,
    ...
    fileprivate func setupRxBindings() {
        Observable.combineLatest(
            ...
            .bind(to: titleDisclaimer.rx.optionalText)
        ...
        viewModel.proposedTitle.asObservable().bind(to: autoGeneratedTitleButton.rx.title).addDisposableTo(disposeBag)
        ...
        viewModel.locationInfo.asObservable().bind(to: setLocationLocationLabel.rx.text).addDisposableTo(disposeBag)
        setLocationButton.rx.tap.bindNext { [weak self] in
            self?.viewModel.openMap()
        }.addDisposableTo(disposeBag)
        
        viewModel.isFreePosting.asObservable().bind(to: freePostingSwitch.rx.value).addDisposableTo(disposeBag)
        freePostingSwitch.rx.value.bind(to: viewModel.isFreePosting).addDisposableTo(disposeBag)
        ...
        viewModel.carMakeName.asObservable().bind(to: carsMakeSelectedLabel.rx.text).addDisposableTo(disposeBag)
        ...
        viewModel.carModelName.asObservable().bind(to: carsModelSelectedLabel.rx.text).addDisposableTo(disposeBag)
        ...
        viewModel.loadingProgress.asObservable().map { $0 == nil }.bind(to: loadingView.rx.isHidden).addDisposableTo(disposeBag)
        viewModel.loadingProgress.asObservable().ignoreNil().bind(to: loadingProgressView.rx.progress).addDisposableTo(disposeBag)

        viewModel.saveButtonEnabled.asObservable().bind(to: sendButton.rx.isEnabled).addDisposableTo(disposeBag)
