...
UINavigationControllerDelegate, FBSDKSharingDelegate, SellProductViewController {
    ...
    func textFieldDidBeginEditing(textField: UITextField) {
        ...
        switch (tag) {
        case .ProductTitle:
            viewModel.userWritesTitle()
        case .ProductPrice, .ProductDescription:
            break
        }
    }
    ...
    func setupUI() {
        ...
        autoGeneratedTitleButton.layer.cornerRadius = 10
    ...
    private func setupRxBindings() {
        ...
        viewModel.titleAutotranslated.asObservable()) { ($0, $1) }
            .map { (titleAutogenerated, titleAutotranslated) in
                if titleAutogenerated && titleAutotranslated {
                    ...
                    return self.viewModel.shouldAskForAutoTitle ? "_ Suggesting title..." : nil
                }
            }
        ...
        viewModel.titleDisclaimerStatus.asObservable().bindNext { [weak self] status in
            switch status {
            case .Completed:
                self?.autoGeneratedTitleButton.hidden = true
                self?.titleDisclaimerActivityIndicator.stopAnimating()
                self?.titleDisclaimer.hidden = false
            case .Ready:
                self?.autoGeneratedTitleButton.hidden = false
                self?.titleDisclaimerActivityIndicator.stopAnimating()
                self?.titleDisclaimer.hidden = true
            case .Loading:
                self?.autoGeneratedTitleButton.hidden = true
                self?.titleDisclaimerActivityIndicator.startAnimating()
                self?.titleDisclaimer.hidden = false
            }
        }.addDisposableTo(disposeBag)
