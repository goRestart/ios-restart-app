...
class EditProductViewModel: BaseViewModel, EditLocationDelegate {
    ...
    var shouldAskForAutoTitle: Bool {
        ...
        print("🚧 🚧 🚧 🚧 🚧 🚧 🚧 🚧 ")
        print(hasTitle)
        print(initialProduct.createdAt)
        guard let creationDate = initialProduct.createdAt else { return !hasTitle }
        return !hasTitle && !creationDate.isOlderThan(Constants.cloudsightTimeThreshold)
    }
    ...
    init(myUserRepository: MyUserRepository, productRepository: ProductRepository, locationManager: LocationManager,
        ...
        self.requestTitleTimer = NSTimer.scheduledTimerWithTimeInterval(5, target: self, selector: #selector(getAutoGeneratedTitle), userInfo: nil, repeats: true)
        self.requestTitleTimer?.fire()
        trackStart()
    }
    ...
    dynamic func getAutoGeneratedTitle() {
        guard shouldAskForAutoTitle else {
