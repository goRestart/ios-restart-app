...
class MLPostListingViewController: BaseViewController, PostListingViewModelDelegate {
    ...
    fileprivate var cameraView: MLPostListingCameraView
    ...
    fileprivate var viewModel: MLPostListingViewModel
    private var shouldHideStatusBar = false
    private let onboardingView = MLPostingOnboardingView()

    // MARK: - Lifecycle

    convenience init(viewModel: MLPostListingViewModel,
    ...
    required init(viewModel: MLPostListingViewModel,
                  forcedInitialTab: Tab?,
                  keyboardHelper: KeyboardHelper) {
        ...
        self.cameraView = MLPostListingCameraView(viewModel: viewModel.postListingCameraViewModel)
    ...
    private func setupMachineLearningOnboardingView() {
        guard !KeyValueStorage.sharedInstance.machineLearningOnboardingShown else { return }
        onboardingView.buttonBlock = { [weak self] in
            ...
            UIView.animate(withDuration: 0.2, animations: {
                ...
                self?.onboardingView.center = self?.cameraView.machineLeaningButton.center ?? CGPoint.zero
            }, completion: { _ in
    ...
    private func setupLoadingStackView() {
        ...
        loadingViewRealEstate = LoadingIndicator(frame: CGRect(x: 0, y: 0, width: MLPostListingViewController.loadingViewWidth, height: MLPostListingViewController.loadingViewHeight))
        loadingViewRealEstate?.layout().height(MLPostListingViewController.loadingViewHeight).width(MLPostListingViewController.loadingViewWidth)
...
// MARK: - MLPostListingCameraViewDelegate

extension MLPostListingViewController: MLPostListingCameraViewDelegate {
...
extension MLPostListingViewController: PostListingGalleryViewDelegate {
...
extension MLPostListingViewController: LGViewPagerDataSource, LGViewPagerDelegate, LGViewPagerScrollDelegate {
    func setupViewPager() {
