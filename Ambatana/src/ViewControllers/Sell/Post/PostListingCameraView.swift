...
class PostListingCameraView: BaseView, LGViewPagerPage {
    ...
    private func setupRX() {
        ...
        previewModeHidden.bind(to: imagePreview.rx.isHidden).addDisposableTo(disposeBag)
        previewModeHidden.bind(to: retryPhotoButton.rx.isHidden).addDisposableTo(disposeBag)
        previewModeHidden.bind(to: usePhotoButton.rx.isHidden).addDisposableTo(disposeBag)
        let captureModeHidden = state.map{ !$0.captureMode }
        captureModeHidden.bind(to: cornersContainer.rx.isHidden).addDisposableTo(disposeBag)
        captureModeHidden.bind(to: switchCamButton.rx.isHidden).addDisposableTo(disposeBag)
        captureModeHidden.bind(to: flashButton.rx.isHidden).addDisposableTo(disposeBag)
        
        viewModel.imageSelected.asObservable().bind(to: imagePreview.rx.image).addDisposableTo(disposeBag)
        ...
        flashMode.map{ $0.imageIcon }.bind(to: flashButton.rx.image).addDisposableTo(disposeBag)

        viewModel.cameraSource.asObservable().subscribeNext{ [weak self] cameraSource in
            self?.cameraWrapper.cameraSource = cameraSource
        }.addDisposableTo(disposeBag)

        viewModel.shouldShowFirstTimeAlert.asObservable().map { !$0 }.bind(to: firstTimeAlertContainer.rx.isHidden).addDisposableTo(disposeBag)
    }
...
extension PostListingCameraView {
    ...
    fileprivate func setupInfoView() {
        ...
        viewModel.infoShown.asObservable().map{ !$0 }.bind(to: infoContainer.rx.isHidden).addDisposableTo(disposeBag)
        viewModel.infoTitle.asObservable().bind(to: infoTitle.rx.text).addDisposableTo(disposeBag)
        viewModel.infoSubtitle.asObservable().bind(to: infoSubtitle.rx.text).addDisposableTo(disposeBag)
        viewModel.infoButton.asObservable().bind(to: infoButton.rx.title).addDisposableTo(disposeBag)
    }
