...
final class ListingCarouselMoreInfoView: UIView, ListingTitleFontDescriptor {
    ...
    // MARK: ListingTitleFontDescriptor Implementation
    
    var titleFont: UIFont {
        return UIFont.productTitleFont
    }
    ...
    var titlePrefixFont: UIFont {
        let fontSize = UIFont.productTitleFont.pointSize
...
private extension ListingCarouselMoreInfoView {
    ...
    func setupContentRx(viewModel: ListingCarouselViewModel) {
        ...
        viewModel.productInfo.asObservable().unwrap().bind { [weak self] info in
            self?.updateTitleLabel(withInfo: info)
            self?.updatePriceLabel(withInfo: info)
    ...
    private func updateTitleLabel(withInfo info: ListingVMProductInfo) {
        let titleViewModel = info.titleViewModel(featureFlags: viewModel?.featureFlags ?? FeatureFlags.sharedInstance)
        let shouldUsePrefixedAttributedTitle = titleViewModel?.shouldUseAttributedTitle ?? false
        if shouldUsePrefixedAttributedTitle {
            titleLabel.attributedText = titleViewModel?.createTitleAttributedString(withFontDescriptor: self)?
                .stringByRemovingLinks
        } else {
            titleLabel.attributedText = info.styledTitle?.stringByRemovingLinks
        }
    }
    
    private func updatePriceLabel(withInfo info: ListingVMProductInfo) {
        guard let priceAttributedString = priceAttributedString(forPrice: info.price,
