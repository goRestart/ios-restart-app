...
extension ProductViewController {
    ...
    private func setupRxBodyBindings() {
        ...
        viewModel.productTitleAutogenerated.asObservable()
            .map { $0 ? LGLocalizedString.productAutoGeneratedTitleLabel : "" }
            .bindTo(nameAutogeneratedDisclaimerLabel.rx_text)
            .addDisposableTo(disposeBag)
        viewModel.productTitleAutoTranslated.asObservable()
            .map { $0 ? LGLocalizedString.productAutoGeneratedTranslatedTitleLabel : "" }
            .bindTo(nameAutotranslatedDisclaimerLabel.rx_text)
            .addDisposableTo(disposeBag)
