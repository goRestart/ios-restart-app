...
class ProductViewModel: BaseViewModel {
    ...
    private func setupRxBindings() {
        ...
        product.asObservable().subscribeNext { [weak self] product in
            ...
            var productTitleAutogenerated = false
            if let productTitle = product.name?.trim where !productTitle.isEmpty {
                strongSelf.productTitle.value = productTitle
            } else if let nameAuto = product.nameAuto?.trim where !nameAuto.isEmpty {
                productTitleAutogenerated = true
                strongSelf.productTitle.value = nameAuto
            } else {
                strongSelf.productTitle.value = nil
            }
            var autoNameTranslated: Bool = false
            if productTitleAutogenerated {
                if let languageCode = strongSelf.countryHelper.countryLanguageForProduct(product) {
                    autoNameTranslated = ( languageCode != "en" )
                }
            }

            strongSelf.productTitleAutogenerated.value = productTitleAutogenerated
            strongSelf.productTitleAutoTranslated.value = autoNameTranslated
