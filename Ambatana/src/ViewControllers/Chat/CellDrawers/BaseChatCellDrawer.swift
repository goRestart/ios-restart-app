...
class BaseChatCellDrawer<T: UITableViewCell where T: ReusableCell>: BaseTableCellDrawer<T>, ChatCellDrawer {
    ...
    */
    func draw(cell: UITableViewCell, message: Message, userName: String, delegate: AnyObject?) {
        guard let myCell = cell as? T else { return }
        draw(myCell, message: message, userName: userName, delegate: delegate)
    }
    
    /**
    Abstract method that should be implemented by the subclasses.
    */
    func draw(cell: T, message: Message, userName: String, delegate: AnyObject?) {}
}
