...
extension Bumper  {
    static func initialize() {
        Bumper.initialize([WebsocketChat.self, NotificationsSection.self, UserReviews.self, ShowNPSSurvey.self, MessageOnFavoriteRound2Mode.self, InterestedUsersMode.self, FiltersReorder.self, FreePostingMode.self, DirectPostInOnboarding.self, ShareButtonWithIcon.self, ProductDetailShareMode.self, PeriscopeChat.self, ChatHeadBubbles.self, ShowLiquidProductsToNewUser.self])
    } 
    ...
    static var showLiquidProductsToNewUser: Bool {
        guard let value = Bumper.valueForKey(ShowLiquidProductsToNewUser.key) else { return true }
        return ShowLiquidProductsToNewUser(rawValue: value)?.asBool ?? true
    } 
}
...
enum ShowLiquidProductsToNewUser: String, BumperFeature  {
    case Yes, No
    static var defaultValue: String { return ShowLiquidProductsToNewUser.Yes.rawValue }
    static var enumValues: [ShowLiquidProductsToNewUser] { return [.Yes, .No]}
    static var values: [String] { return enumValues.map{$0.rawValue} }
