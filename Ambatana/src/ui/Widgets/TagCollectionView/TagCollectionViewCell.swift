...
enum TagCollectionViewCellStyle {
    ...
    var font: UIFont {
        ...
        case .blackBackgroundWithCross:
            return UIFont.systemFont(size: 15)
        }
    }
    
    var textColor: UIColor {
        switch self {
        case .blackBackground, .blackBackgroundWithCross:
            return UIColor.white
        case .whiteBackground:
    ...
    var borderColor: UIColor {
        switch self {
        case .blackBackground, .whiteBackground, .blackBackgroundWithCross:
            return .clear
        case .grayBorder:
            return .grayLight
        }
    ...
    var padding: UIEdgeInsets {
        ...
        case .blackBackgroundWithCross:
            return UIEdgeInsets(top: 7, left: 15, bottom: 7, right: 34)
        }
    }
    
    var containsCross: Bool {
        switch self {
        case .blackBackground, .whiteBackground, .grayBorder:
            return false
        case .blackBackgroundWithCross:
            return true
        }
...
protocol TagCollectionConfigurable {
    func setupWith(style: TagCollectionViewCellStyle)
    func configure(with tag: String)
}

final class TagCollectionViewCell: UICollectionViewCell, ReusableCell, TagCollectionConfigurable {
    
    var style: TagCollectionViewCellStyle = TagCollectionViewCellStyle.blackBackground
    private let tagLabel: UIRoundedLabelWithPadding = {
