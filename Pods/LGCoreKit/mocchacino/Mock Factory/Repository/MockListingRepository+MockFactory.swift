...
extension MockListingRepository: MockFactory {
    public static func makeMock() -> Self {
        ...
        //mockProductRepository.carResult = ListingsResult(value: MockProduct.makeMocks(count: Int.makeRandom(min: 0, max: 10)).map { Listing.product($0) })
        ...
        mockProductRepository.markAsSoldVoidResult = ListingVoidResult(value: Void())
        mockProductRepository.markAsUnsoldVoidResult = ListingVoidResult(value: Void())
