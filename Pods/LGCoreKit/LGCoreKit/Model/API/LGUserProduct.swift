...
struct LGUserProduct: UserProduct {
...
extension LGUserProduct : Decodable {
    ...
    static func decode(_ j: JSON) -> Decoded<LGUserProduct> {
        let init1 = curry(LGUserProduct.init)
            <^> j <|? "id"
            <*> j <|? "name"
            <*> j <|? "avatar_url"
        ...
        if let error = result.error {
            logMessage(.error, type: CoreLoggingOptions.parsing, message: "LGUserProduct parse error: \(error)")
        }
