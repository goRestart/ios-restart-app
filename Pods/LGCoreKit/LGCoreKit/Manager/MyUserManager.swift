...
public class MyUserManager: LocationManagerDelegate {
    ...
    private let userLogInEmailService: UserLogInEmailService
    private let userLogInFBService: UserLogInFBService
    private let fbUserInfoRetrieveService: FBUserInfoRetrieveService
    private let userLogOutService: UserLogOutService
    
    private let userPasswordResetService: UserPasswordResetService
    
    private let myUserRetrieveService: MyUserRetrieveService
    private let userSaveService: UserSaveService
    private let installationSaveService: InstallationSaveService
    ...
    private var myUserDAO: MyUserDAO
    ...
    public convenience init() {
        let userSignUpService = PAUserSignUpService()
        let userLogInEmailService = PAUserLogInEmailService()
        let userLogInFBService = PAUserLogInFBService()
        let fbUserInfoRetrieveService = FBUserInfoRetrieveService()
        let userLogOutService = PAUserLogOutService()
        
        let userPasswordResetService = PAUserPasswordResetService()
        
        let myUserRetrieveService = PAMyUserRetrieveService()
        let userSaveService = PAUserSaveService()
        ...
        self.init(
            ...
            userLogInEmailService: userLogInEmailService,
            userLogInFBService: userLogInFBService,
            fbUserInfoRetrieveService: fbUserInfoRetrieveService,
            userLogOutService: userLogOutService,
            userPasswordResetService: userPasswordResetService,
            myUserRetrieveService: myUserRetrieveService,
            userSaveService: userSaveService,
            installationSaveService: installationSaveService,
            postalAddressRetrievalService: postalAddressRetrievalService,
    ...
    public required init(userSignUpService: UserSignUpService,
        userLogInEmailService: UserLogInEmailService,
        userLogInFBService: UserLogInFBService,
        fbUserInfoRetrieveService: FBUserInfoRetrieveService,
        userLogOutService: UserLogOutService,
        ...
        // Init
        // > Services
        self.userSignUpService = userSignUpService
        self.userLogInEmailService = userLogInEmailService
        self.userLogInFBService = userLogInFBService
        self.fbUserInfoRetrieveService = fbUserInfoRetrieveService
        self.userLogOutService = userLogOutService
        
        self.userPasswordResetService = userPasswordResetService
        
        self.myUserRetrieveService = myUserRetrieveService
        ...
        if let myUserId = myUser()?.objectId {
    ...
    */
    ...
    /**
        Saves the user if it's new or retrieves it.
    
        - parameter result: The closure containing the result.
    */
    public func saveOrRetrieveMyUser(completion: UserSaveServiceCompletion?) {
        guard let myUser = myUser(), let sessionToken = myUser.sessionToken else {
            completion?(UserSaveServiceResult(error: .Internal))
            return
        }
        
        // If it's saved then retrieve it
        if let myUserId = myUser.objectId {
            myUserRetrieveService.retrieveMyUserWithSessionToken(sessionToken, myUserId: myUserId) { (result: MyUserRetrieveServiceResult) -> Void in
                // Success
    ...
    */
    public func signUpWithEmail(email: String, password: String, publicUsername: String, completion: UserSignUpServiceCompletion?) {
        ...
        logout { (_) -> Void in
            ...
            // 2. Sign up
            self.userSignUpService.signUpUserWithEmail(email, password: password, publicUsername: publicUsername) { (myResult: UserSignUpServiceResult) in
                
                // Succeeded
                if let myUser = myResult.value {
                    
                    // 3a. If anonymous then logout
                    if myUser.isAnonymous {
                        self.userLogOutService.logOutUser(myUser) { (logoutResult: UserLogOutServiceResult) -> Void in
                            completion?(UserSignUpServiceResult(error: .Internal))
                        }
                    }
                    // 3b. Otherwise, set it up
                    else {
    ...
    */
    public func logInWithEmail(email: String, password: String, completion: UserLogInEmailServiceCompletion?) {
        ...
        logout { (_) -> Void in
            // 2. Login
            self.userLogInEmailService.logInUserWithEmail(email, password: password) { (myResult: UserLogInEmailServiceResult) in
                // Succeeded
                if let user = myResult.value {
                    ...
                    // 3a. If scammer then logout & notify
                    if isScammerUser {
                    ...
                    // 3b. If anonymous then logout & notify
                    else if user.isAnonymous {
                        self.userLogOutService.logOutUser(user) { (logoutResult: UserLogOutServiceResult) -> Void in
                            completion?(UserLogInEmailServiceResult(error: .Internal))
                        }
                    }
                    // 3c. Otherwise it's a regular user, we're done
                    else {
                        self.setupAfterSessionSuccessful()
                        completion?(myResult)
                    }
    ...
    */
    public func logInWithFacebook(completion: UserLogInFBCompletion?) {
        ...
        logout { (_) -> Void in
            ...
            self.userLogInFBService.logInByFacebooWithCompletion { (myResult: UserLogInFBServiceResult) in
                ...
                if let user = myResult.value, let _ = user.objectId {
                    ...
                    // 3b. If anonymous then logout & notify
                        ...
                        self.userLogOutService.logOutUser(user) { (logoutResult: UserLogOutServiceResult) -> Void in
                            completion?(UserLogInFBResult(error: .Internal))
                        }
                    }
                    // 3c. Otherwise it's a regular user, then retrieve the FB info
                    else {
                        // 4. Retrieve the FB Info
