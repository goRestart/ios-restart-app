// @ahl: Modified from Alamofire with writeEncodedDataToDisk (synch)
...
*/
public class LGMultipartFormData {
    ...
    */
    ...
    /**
        Writes the appended body parts into the given file URL synchronously.
    ...
    */
    public func writeEncodedDataToDisk(fileURL: NSURL) -> NSError? {
        if let bodyPartError = bodyPartError {
        ...
        if let path = fileURL.path where NSFileManager.defaultManager().fileExistsAtPath(path) {
            let failureReason = "A file already exists at the given file URL: \(fileURL)"
            return errorWithCode(NSURLErrorBadURL, failureReason: failureReason)
        } else if !fileURL.fileURL {
            let failureReason = "The URL does not point to a valid file: \(fileURL)"
            return errorWithCode(NSURLErrorBadURL, failureReason: failureReason)
        }
