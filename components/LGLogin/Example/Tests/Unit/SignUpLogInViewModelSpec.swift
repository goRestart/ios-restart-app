...
class SignUpLogInViewModelSpec: QuickSpec, MockBaseViewModel {
    ...
    override func spec() {
        describe("SignUpLogInViewModelSpec") {
            ...
            let termsAndConditionsEnabled = false
            ...
            beforeEach {
                ...
                sut = SignUpLogInViewModel(sessionManager: sessionManager, installationRepository: installationRepository,
                    ...
                    fbLoginHelper: fbLoginHelper, tracker: tracker,
                    locale: locale, source: .install, action: .signup, termsAndConditionsEnabled: termsAndConditionsEnabled)
                sut.delegate = self
                sut.navigator = self
                ...
                self.state.delegateReceivedHideLoading = false
                self.state.delegateReceivedShowAlert = false
            ...
            describe("initialization") {
                context("common") {
                    ...
                    it("maps feature flags' terms and conditions enabled") {
                        expect(sut.termsAndConditionsEnabled) == termsAndConditionsEnabled
                    }
                }
            ...
            describe("login with email") {
                context("successful") {
                    ...
                    beforeEach {
                        ...
                        sut.logIn()
                        expect(self.state.delegateReceivedHideLoading).toEventually(beTrue())
                    }
                ...
                describe("log in process") {
                    ...
                    context("with email non-valid") {
                        ...
                        it("shows error message") {
                            expect(self.state.delegateReceivedShowAutoFadingMessage).toEventually(beTrue())
                        }
                    }
                    context("with short password") {
                        beforeEach {
                ...
                describe("send log in") {
                    ...
                    context("log in fails once with unauthorized error") {
                        ...
                        it("does not call show alert in the delegate to suggest reset pwd") {
                            expect(self.state.delegateReceivedShowAlert) == false
                        }
                        it("calls show and hide loading in delegate") {
                            expect(self.state.delegateReceivedShowLoading).toEventually(beTrue())
                            expect(self.state.delegateReceivedHideLoading).toEventually(beTrue())
                        }
                    }
                    ...
                    context("log in fails twice with unauthorized error") {
                        ...
                        it("calls show alert in the delegate to suggest reset pwd") {
                            expect(self.state.delegateReceivedShowAlert).toEventually(beTrue())
                        }
                        it("calls show and hide loading in delegate") {
                            expect(self.state.delegateReceivedShowLoading).toEventually(beTrue())
                            expect(self.state.delegateReceivedHideLoading).toEventually(beTrue())
