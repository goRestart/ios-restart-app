...
final class AnalyticsBuySell24hMiddlewareSpec: QuickSpec {
    override func spec() {
        describe("AnalyticsBuySell24hMiddleware") {
            var tracker: MockTracker!
            var keyValueStorage: MockKeyValueStorage!
            var sut: AnalyticsBuySell24hMiddleware!

            beforeEach {
                tracker = MockTracker()
                keyValueStorage = MockKeyValueStorage()
                keyValueStorage.currentUserProperties = UserDefaultsUser()
                sut = AnalyticsBuySell24hMiddleware(keyValueStorage: keyValueStorage)
            }
            ...
            describe("process listing first message event") {
                ...
                context("when first run date is before 24h and did not track a buyer lister 24h event") {
                    beforeEach {
                        keyValueStorage[.firstRunDate] = Date(timeIntervalSinceNow: -TimeInterval.make(minutes: 1))
                        keyValueStorage.userTrackingProductBuySellComplete24hTracked = false
                        sut.process(event: event,
                                    trackNewEvent: tracker.trackEvent)
                    }
                    it("calls back to track a buyer lister 24h event") {
                        expect(tracker.trackedEvents[0].name) == .buyerLister24h
                    }
                    ...
                    it("updates userTrackingProductSellComplete24hTracked") {
                        expect(keyValueStorage.userTrackingProductBuySellComplete24hTracked) == true
                    }
                }

                context("when first run date is after 24h and did not track a buyer lister 24h event") {
                    beforeEach {
                ...
                context("when first run date is before 24h and did track a buyer lister 24h event") {
                    beforeEach {
                        keyValueStorage[.firstRunDate] = Date(timeIntervalSinceNow: -TimeInterval.make(minutes: 1))
                ...
                context("when first run date is after 24h and did track a buyer lister 24h event") {
                    beforeEach {
                        keyValueStorage[.firstRunDate] = Date(timeIntervalSinceNow: -TimeInterval.make(days: 2))
                        keyValueStorage.userTrackingProductBuySellComplete24hTracked = true
                        sut.process(event: event,
